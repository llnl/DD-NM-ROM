...
class SubdomainROM(object):
  ...
  def __init__(
    ...
    # precompute transpose of interface cmat, used only for strong constraints
    self.cmat_interface_t = None
    if self.constraint_type == "strong":
      if torch.is_tensor(self.cmat["interface"]):
        self.cmat_interface_t = torch.t(self.cmat["interface"])
      else:
        self.cmat_interface_t = self.cmat["interface"].transpose()
  ...
  def res_jac(
    ...
    for e_k in ("interior", "interface"):
      jac[e_k] = jac[e_k] @ (dec_jac[e_k].to_dense() if bkd.is_torch_backend() else dec_jac[e_k])
  ...
  def compute_cres_cjac(
    ...
    if (self.constraint_type == "weak"):
      if (self.hr_active):
        ...
        cx = cat_func([uv[celem][x_k] for x_k in ("u", "v")])
