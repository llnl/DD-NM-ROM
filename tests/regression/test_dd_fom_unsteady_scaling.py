...
def test_ddfom_distributed_unsteady():
    ...
    profiler = Profiler()
    profile_out = "./profiles/test_dd_fom_unsteady_scaling_profile_gpu"
    if bkd.distributed():
    ...
    profiler.stop()
    with open(profile_out + "_fom{}x{}.out".format(n_sub_x, n_sub_y), 'w') as file:
        profiler.print(file)
...
if __name__ == "__main__":
    setup_module(None)
    test_ddfom_distributed_unsteady()
